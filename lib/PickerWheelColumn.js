...
class PickerWheelColumn extends Component<void, Props, State> {
    ...
    _updateItemsAndMargin(difference) {
        ...
        if (difference > 0) {
            ...
            this.setState({
                items: [
                    ...items.slice(difference),
                    shiftedItems,
                ],
                marginTop: (this.currentIndex - this.middleIndex) * this.itemHeight + this.itemHeight / 2,
            });
        } else if (difference < 0) {
            const shiftedItems = items.slice(items.length + difference, -difference);
            this.currentIndex += difference;
            this.setState({
                items: [
                    shiftedItems,
                    ...items.slice(0, items.length + difference),
                ],
                marginTop: (this.currentIndex - this.middleIndex) * this.itemHeight + this.itemHeight / 2,
            });
        }
    ...
    _moveTo(obj, currentIndex, direction) {
        ...
        const fixedAnimationTime = 200;
        const accelerationRate = -(this.velocity / fixedAnimationTime); // units per ms for decelleration
        ...
        const additionalIndexesToTravel = Math.floor(unitsToTravel / this.itemHeight);
        const virtualCurrentIndex = (additionalIndexesToTravel * direction) + currentIndex;
