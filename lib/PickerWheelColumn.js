...
class PickerWheelColumn extends Component<void, Props, State> {
    constructor(props) {
        ...
        this.middleIndex = Math.floor(this.items.length / 2);
    ...
    _moveToNext(direction) {
        const value = this.items[this.middleIndex];
    ...
    _moveTo(obj, currentIndex) {
        ...
        setTimeout(() => {
            ...
            this.props.onSelect(this.items[this.middleIndex]);
     ...
     * @return {Object}      JSX对象
     */
    renderPickerWheelItem(value, index) {
        const className =
