...
class PickerWheelColumn extends Component<void, Props, State> {
    ...
    _moveTo(obj, currentIndex, direction) {
        ...
        if (this.estimatedAccelerationRate !== this.accelerationRate) {
            ...
            const estimatedEndTranslateY = this.state.translateY - estimatedDistTravelled;

            // target is next mod item height
            const targetDistTravelled = estimatedEndTranslateY
                - (estimatedEndTranslateY % this.itemHeight)
                + this.itemHeight
