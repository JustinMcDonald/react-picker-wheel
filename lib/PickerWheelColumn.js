    ...
    _moveTo(obj, currentIndex, direction) {
        ...
        */
        ...
        if (this.estimatedAccelerationRate !== this.accelerationRate) {
            ...
            const estimatedDistTravelled = (this.velocity * FIXED_SPIN_ANIMATION_TIME) * numberOfAccelerationEvents + (0.5 * (this.estimatedAccelerationRate * FIXED_SPIN_ANIMATION_TIME) * Math.pow(numberOfAccelerationEvents, 2));
            //const totalEstimatedDistTravelled = estimatedDistTravelled * FIXED_SPIN_ANIMATION_TIME;
            const totalTargetDistTravelled = totalEstimatedDistTravelled - (totalEstimatedDistTravelled % this.itemHeight);
            //const targetDistTravelled = totalTargetDistTravelled / FIXED_SPIN_ANIMATION_TIME;
            const predictedAccelerationRate = ((totalEstimatedDistTravelled - ((this.velocity * FIXED_SPIN_ANIMATION_TIME) * numberOfAccelerationEvents)) * 2) / (Math.pow(numberOfAccelerationEvents, 2) * FIXED_SPIN_ANIMATION_TIME);
            console.log({
