...
const FIXED_SPIN_ANIMATION_TIME = 100;
    ...
    _moveTo(obj, currentIndex, direction) {
        ...
        */
        ...
        if (this.estimatedAccelerationRate !== this.accelerationRate) {
            ...
            const estimatedDistTravelled = this.velocity * numberOfAccelerationEvents + (0.5 * this.estimatedAccelerationRate * Math.pow(numberOfAccelerationEvents, 2));
            const targetDistTravelled = estimatedDistTravelled - (estimatedDistTravelled % this.itemHeight);
        ...
        this.setState((state, props) => {
            return { translateY: state.translateY - this.velocity };
        });
        ...
        this.spinTimeout = setTimeout(() => {
            ...
            if (direction >= 0 && this.velocity + this.accelerationRate <= 0 ||
                direction <= 0 && this.velocity + this.accelerationRate >= 0) {
                const animationTimeLeft = FIXED_SPIN_ANIMATION_TIME * Math.abs(this.velocity / this.accelerationRate);
